...
from apps.agent.tools import workflow_tool
...
async def check_availability(
    ...
    """Check available appointment slots for a doctor. Needs doctor_id and date.
    ...
    *date* (start) and *date_to* (end, inclusive).
    ...
    If you only have the doctor's NAME, call search_doctors first to obtain
    the doctor_id, then call this tool with it."""
    ...
    used_doctor_id = doctor_id
    if not used_doctor_id:
        if doctor_name:
            return Command(update={"messages": [ToolMessage(
                content=json.dumps({"error": f"Doctor '{doctor_name}' was not resolved. Call search_doctors first with the doctor's name, then retry with the returned doctor_id."}),
                tool_call_id=runtime.tool_call_id if runtime else "",
            )]})
        return Command(update={"messages": [ToolMessage(content='{"error": "doctor_id or doctor_name is required."}', tool_call_id=runtime.tool_call_id if runtime else "")]})

    result = await workflow_tool.check_availability(used_doctor_id, date, cid, date_to=date_to)
    try:
        content = json.dumps(result, ensure_ascii=False)
    except Exception:
        content = str(result)
